 ...
 */
object PeopleRecentCountTop {
  def filename:String  = "zhunian_PeopleRecentCountTop_";
  def main(args:Array[String]) {
    val conf = new SparkConf().setAppName("zhunian_PeopleRecentCountTop");
    ...
    val pcDate = textFile.map(line => line.split("-")).
     ...
     filter(line => line(0).substring(4,7).toInt > no).
       ...
       flatMap(line => (line(1).split(","))).//把人员单个拆分到一个数组
       map(line => (line,1)).
      reduceByKey((a, b) => a + b).
      sortBy(_._2, false)//根据数量降序
